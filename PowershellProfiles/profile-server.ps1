...
# Check if IDs already exist in registry, otherwise generate new ones
...
if (!$script:ServiceTag) {
    ...
    Set-ItemProperty -Path $regPath -Name "ServiceTag" -Value $script:ServiceTag
}
...
if (!$script:SerialNumber) {
    ...
    Set-ItemProperty -Path $regPath -Name "SerialNumber" -Value $script:SerialNumber
}
...
if (!$script:SystemUUID) {
    ...
    Set-ItemProperty -Path $regPath -Name "SystemUUID" -Value $script:SystemUUID
}
...
if (!$script:DiskSerial) {
    ...
    Set-ItemProperty -Path $regPath -Name "DiskSerial" -Value $script:DiskSerial
}
...
# Read system configuration from registry
$script:SystemManufacturer = Get-RegistryValue -Path $biosRegPath -Name "SystemManufacturer" -Default "Dell Inc."
$script:SystemProductName = Get-RegistryValue -Path $biosRegPath -Name "SystemProductName" -Default "PowerEdge R640"
$script:BIOSVendor = Get-RegistryValue -Path $biosRegPath -Name "BIOSVendor" -Default "Dell Inc."
$script:BIOSVersion = Get-RegistryValue -Path $biosRegPath -Name "BIOSVersion" -Default "2.17.0"
$script:BIOSReleaseDate = Get-RegistryValue -Path $biosRegPath -Name "BIOSReleaseDate" -Default "05/15/2024"
$script:BaseBoardManufacturer = Get-RegistryValue -Path $biosRegPath -Name "BaseBoardManufacturer" -Default "Dell Inc."
$script:BaseBoardProduct = Get-RegistryValue -Path $biosRegPath -Name "BaseBoardProduct" -Default "0CNCJW"
...
if ($script:BIOSReleaseDate -match "(\d{2})/(\d{2})/(\d{4})") {
    ...
    $script:BIOSReleaseDateWMI = "20240515000000.000000+000"
}
...
if ($script:BIOSVersion -match "(\d+)\.(\d+)") {
    ...
    $script:SMBIOSMajorVersion = 2
    $script:SMBIOSMinorVersion = 17
}
...
function global:Get-WmiObject {
    ...
    if ($Class -eq "Win32_BIOS" -or $Query -like "*Win32_BIOS*") {
        return [PSCustomObject]@{
            ...
            BiosCharacteristics = @(4, 7, 15, 16, 19, 26, 27, 28, 29, 32, 33, 40, 42, 43)
    ...
    if ($Class -eq "Win32_DiskDrive" -or $Query -like "*Win32_DiskDrive*") {
        ...
        foreach ($disk in $realDisks) {
            if ($disk.Model -match "VMware|VBOX|QEMU|Virtual|Xen|Microsoft Virtual Disk") {
                $disk.Model = "DELL PERC H730P"
                $disk.Caption = "DELL PERC H730P SCSI Disk Device"
    ...
    if ($Class -eq "Win32_NetworkAdapter" -or $Query -like "*Win32_NetworkAdapter*") {
        ...
        foreach ($adapter in $realAdapters) {
            if ($adapter.Name -match "VMware|vmxnet|Virtual") {
                $adapter.Name = "Intel(R) Ethernet Connection I219-LM"
                $adapter.Description = "Intel(R) Ethernet Connection I219-LM"
                $adapter.Manufacturer = "Intel Corporation"
                $adapter.ProductName = "Intel(R) Ethernet Connection I219-LM"
            }
        }
    ...
    if ($Class -eq "Win32_NetworkAdapterConfiguration" -or $Query -like "*NetworkAdapterConfiguration*") {
        ...
        foreach ($config in $realConfigs) {
            if ($config.Description -match "VMware|vmxnet|Virtual") {
                $config.Description = "Intel(R) Ethernet Connection I219-LM"
                $config.ServiceName = "e1dexpress"
            }
        }
    ...
    if ($Class -eq "Win32_VideoController" -or $Query -like "*Win32_VideoController*") {
        ...
        if ($realVideo.Name -match "VMware|SVGA") {
            $realVideo.Name = "Microsoft Basic Display Adapter"
            $realVideo.Description = "Microsoft Basic Display Adapter"
            $realVideo.VideoProcessor = "Microsoft Basic Display Adapter"
            $realVideo.AdapterCompatibility = "Microsoft Corporation"
        }
    ...
    if ($Class -eq "Win32_IDEController" -or $Query -like "*IDEController*") {
        ...
        foreach ($ide in $realIDE) {
            if ($ide.Name -match "VMware|Virtual") {
                $ide.Name = "Intel(R) C600 series chipset IDE Controller"
    ...
    if ($Class -eq "Win32_SCSIController" -or $Query -like "*SCSIController*") {
        ...
        foreach ($scsi in $realSCSI) {
            if ($scsi.Name -match "VMware|Virtual") {
                $scsi.Name = "Dell PERC H730P Adapter"
                $scsi.Manufacturer = $script:SystemManufacturer
                $scsi.Description = "Dell PERC H730P Adapter"
            }
        }
...
# Write-Host "WMI Hooks Loaded - Dell hardware spoofing active" -ForegroundColor Green
